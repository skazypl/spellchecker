...
static void createNode_test(void** state) {
    ...
    n = createNode(MAX_CHILD + 1, 0, NULL);
    assert_true(n == NULL);
}
...
static void nodeComp_test(void** state) {
    ...
    // assert_int_equal(nodeComp(n, m), 1);
    // assert_int_equal(nodeComp(m, n), -1);
    //assert_int_equal(nodeComp(n, n), 0);

    // free(m);
    // m = createNode(0, 'a', NULL);
    // assert_int_equal(nodeComp(n, m), 0);
    // assert_int_equal(nodeComp(m, n), 0);
    free(n);
    free(m);
